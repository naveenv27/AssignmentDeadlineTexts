...
(defn formatted-hours [hours]
  (str "from " (format-hour (get hours :start)) " to "
...
(defn answer-question [conversation {:keys [args]}]
  (cond (empty? args) [[] "You did not provide an answer."]
    ...
    :else [[(into {} (action-send-msg (conversation :asker)
...
(def assignments
  {:CS2201 "\"assignment 5 due 10/23 at 8am\""})


(defn teacher-register [teacher class id info]
  (action-insert [:teacher class id] info))

(defn teacher-question-msg [teacher question-words user-id]
  (str "Asking " (count teacher) " teacher for an answer to: \""
       (string/join " " question-words) "\".............." (get assignments :CS2201) "."))

(defn add-teacher [teacher {:keys [args user-id]}]
  [[(teacher-register teacher (first args) user-id {})]
   (str user-id " is now an teacher in course " (first args) ".")])

(defn ask-teacher [teacher {:keys [args user-id]}]
  (cond (empty? teacher) [[] "There are no teachers in that class."]
    (empty? (rest args)) [[] "You must ask a valid question."]
    :else [[
            (into {} (action-inserts [:conversations] teacher {:last-question (clojure.string/join " " (rest args)) :asker user-id}))]
           (teacher-question-msg teacher (rest args) user-id)]))



(defn add-asgn [teacher {:keys [args user-id]}]
  (cond (empty? args) [[] "You did not provide any info."]
    :else [(assignments (keyword user-id) (clojure.string/join " " (rest args)))
           "Your assignment info has been updated."]))
...
(defn teacher-in-class-query [state-mgr pmsg]
  (let [[class]  (:args pmsg)]
    (list! state-mgr [:teacher class])))
...
(def queries
  {"expert" experts-on-topic-query
   ...
   "teacher" teacher-in-class-query
   "class"  teacher-in-class-query
   "add"    teacher-in-class-query})
...
(defn create-router [routes]
  (fn [msg]
    (if (contains? routes (msg :cmd))
      (get routes (get msg :cmd) (msg :args))
      (get routes "default"))))
